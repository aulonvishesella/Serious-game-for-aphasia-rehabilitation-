...
* Using `Feature Driven Development (FDD)` consistently throughout development, with each feature being validated by a speech therapist. By doing so, I was able to adjust the feature according to therapists’ suggestions, which was pivotal in making sure that the finished product can provide the desired therapeutic effects.
